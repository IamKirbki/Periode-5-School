...
router.post('/api/pokemon/favourite', async function (req, res, next) {
    const instertQuery = "INSERT INTO user_pokemon_favourite (user_id, pokemon_id) VALUES (?, ?)";
    const deleteQuery = "DELETE FROM user_pokemon_favourite WHERE user_id = ? AND pokemon_id = ?"
    const selectQuery = 'SELECT * FROM user_pokemon_favourite WHERE user_id = ? AND pokemon_id = ?'
